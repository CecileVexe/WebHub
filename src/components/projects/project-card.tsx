...
>
  ...
  <div className="relative h-56 overflow-hidden">
    ...
    {/* Overlay on Hover (now accessible by keyboard) */}
    <div
      className={`absolute inset-0 bg-gradient-to-t ${colors.overlay} opacity-0 group-hover:opacity-100 focus-within:opacity-100 transition-opacity duration-300 flex items-center justify-center`}
    >
      <button
        className="px-6 py-3 bg-white text-gray-900 rounded-full transform scale-0 group-hover:scale-100 group-focus-within:scale-100 transition-transform duration-300 flex items-center gap-2 shadow-xl focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-purple-600"
    ...
    <div className="absolute top-4 left-4">
      <span
        className={`${colors.badge} text-white px-4 py-2 rounded-full text-sm shadow-lg`}
    ...
    {/* Like Button — fully accessible */}
    <button
      onClick={() => setIsLiked(!isLiked)}
      className="absolute top-4 right-4 w-10 h-10 bg-white/90 backdrop-blur-sm rounded-full flex items-center justify-center shadow-lg hover:scale-110 transition-transform focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-pink-500"
      aria-pressed={isLiked}
      aria-label={
        isLiked
          ? `Retirer des favoris : ${project.title}`
          : `Ajouter aux favoris : ${project.title}`
      }
    >
      <Heart
        className={`w-5 h-5 ${
  ...
  <div className="p-6">
    <h3
      ...
      className="text-gray-900 mb-3 group-hover:text-transparent group-hover:bg-gradient-to-r group-hover:from-purple-600 group-hover:via-pink-600 group-hover:to-orange-600 group-hover:bg-clip-text transition-all"
    >
      {project.title}
    </h3>
    ...
    {/* Tags */}
    <div className="flex flex-wrap gap-2 mb-5" aria-label="Étiquettes du projet">
